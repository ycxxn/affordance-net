...
if [ -z "${DOPE_ID}" ]; then
    ...
    xhost +local:root
    docker run --gpus all -it --privileged --network=host -v /home/rvl224/文件/affordance-net:/workspace:rw -v /tmp/.X11-unix:/tmp/.X11-unix:rw --env="DISPLAY" --name=${CONTAINER_NAME} affordancenet bash
else
